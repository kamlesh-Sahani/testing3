...
const AppContext = ({ children }) => {
    ...
    useEffect(() => {
        async function authUser() {
            try {
                let token = localStorage.getItem('token')
                let response = await axios.get('https://new-deploy-s3gw.onrender.com/api/auth/verify', {
                    headers: {
                        token: token
                    }
                })
                ...
                if (details.role === 'student') {
                    let userResponse = await axios.get('https://new-deploy-s3gw.onrender.com/api/student/findStudent', {
                        params: {
                            id: userId
                        }
                    })
                    ...
                    let userResponse = await axios.get('https://new-deploy-s3gw.onrender.com/api/teacher/findTeacher', {
                        params: {
                            id: userId
                        }
                    })
